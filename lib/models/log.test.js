...
describe('Log class', () => {
  ...
  it('should update a log when given an id and updated info', async() => {
    ...
    const updatedLog = await Log.update(firstInsertedLog.id, { recipeId: firstInsertedLog.recipeId, dateOfEvent: '05/17/1996', notes: 'just an okay day', rating: 6 });

    expect(updatedLog).toEqual({ id: firstInsertedLog.id, recipeId: Number(firstInsertedLog.recipeId), dateOfEvent: '05/17/1996', notes: 'just an okay day', rating: 6 });
  });
